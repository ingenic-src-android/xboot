...
#include "asm/jz_mem_nand_configs/DDR3_H5TQ2G83EFR-PBC.h"
/* NAND FLASH chip*/
...
#include "asm/jz_mem_nand_configs/NAND_H27UAG8T2BTR.h"
...
#elif (KERNEL_NAND_START == 16)	/* start from 8M */
 ...
 */
#define CYCLE_BEGIN_PIN			GPIO_NULL
#define CYCLE_STOP_PIN			GPIO_NULL

#define GPIO_BOOT_SEL0			(32 * 3 + 17)	/* GPD17 for vehicle */
#define GPIO_BOOT_SEL1		 	(32 * 3 + 18)	/* GPD18 for vehicle */
#define GPIO_KEY_WAKEUP			(32 * 0 + 30)	/* GPA30 WAKEUP for vehicle */
#define GPIO_KEY_MAIN			(32 * 5 + 10)	/* vehicle need def */
#ifdef CONFIG_NPM702_V_1_1
#define GPIO_KEY_VOLUME_INC		(32 * 2 + 28)	/* vehicle need def */
#else
#define GPIO_KEY_VOLUME_INC		(32 * 2 + 28)	/* vehicle need def */
#endif

#define GPIO_KEY_VOLUME_DEC		GPIO_BOOT_SEL1	/* vehicle need def */

#define GPIO_KEY_BACK			(32 * 5 + 9)	/* vehicle need def */
#define GPIO_KEY_MENU			(GPIO_KEY_MAIN)	/* vehicle need def */
#define GPIO_KEY_BOOT			(GPIO_KEY_BACK)	/* vehicle need def */

#define GPIO_CHARGE_DETECT		GPIO_NULL	/* null_pin for vehicle */
#define GPIO_CHARGE_LEVEL		(0)
#define GPIO_USB_DETECT			(32 * 5 + 13)	/* GPF13----vehicle */
#define GPIO_USB_INSERT_LEVEL	(0)

#define GPIO_DC_DETECT			(32 * 4 + 3)	/* vehicle need def */
#define GPIO_DC_INSERT_LEVEL	(1)

#define GPIO_AMP_ENABLE			(32 * 4 + 6)	/* vehicle need def */
#define GPIO_AMP_MUTE_LEVEL		(0)


#define GPIO_HP_MUTE			(32 * 3 + 13)	/* vehicle need def */
#define GPIO_HP_MUTE_LEVEL		(1)

#define GPIO_HDMI_RESET			GPIO_NULL	/* null_pin for vehicle */
#define GPIO_HDMI_ENABLE_LEVEL	(0)

#define GPIO_HDMI_INT			GPIO_NULL	/* null_pin for vehicle */
#define GPIO_HDMI_INT_LEVEL		(0)

#define GPIO_CAMERA_RESET		(32 * 1 + 18)	/* GPB18 for vehicle */
#define GPIO_CAMERA_RESET_LEVEL	(0)

#define GPIO_CAMERA_ENABLE		(32 * 1 + 19)	/* GPB19 for vehicle */
#define GPIO_CAMERA_ENABLE_LEVEL	(0)

#define GPIO_GPS_POWER_ENABLE		(32 * 5 + 10)	/* vehicle need def */
#define GPIO_GPS_POWER_LEVEL	(1)

#define GPIO_GPS_OSC_ENABLE		(32 * 1 + 24)	/* vehicle need def */
#define GPIO_GPS_OSC_LEVEL		(1)

#define GPIO_TFLASH_DETECT		(32 * 1 + 22)	/* vehicle need def */
#define GPIO_TFLASH_INSERT_LEVEL	(1)

#define GPIO_ECOMPASS_DRDY		(32 * 3 + 23)	/* vehicle need def */
#define GPIO_ECOMPASS_DRDY_LEVEL	(1)

#define GPIO_GSENSOR_INT1		-1		/* null */
#define GPIO_GSENSOR_INT2		-1		/* null */

#define GPIO_CTP_INT			GPIO_NULL	/* gpio_null for vehicle now*/
#define GPIO_CTP_WAKEUP			(32 * 5 + 18)	/* gpio_null for vehicle now*/

#define GPIO_BT_RESET			-1		/* null for vehicle */
#define GPIO_BT_RESET_LEVEL		(0)

#define GPIO_BT_INT			-1		/* null for vehicle */
#define GPIO_BT_WAKEUP			-1		/* null for vehicle */

#define GPIO_BT_POWER			-1		/* null for vehicle */
#define GPIO_BT_POWER_LEVEL		(1)

#define GPIO_WLAN_POWER			-1		/* null for vehicle */
#define GPIO_WLAN_POWER_LEVEL		(1)

#define GPIO_WLAN_RESET			-1		/* null for vehicle */
#define GPIO_WLAN_RESET_LEVEL		(0)

#define GPIO_WLAN_INT			-1		/* null for vehicle */
#define GPIO_WLAN_WAKEUP		-1		/* null for vehicle */

#define GPIO_PMU_ENABLE			-1		/* null for vehicle */
#define GPIO_PMU_ENABLE_LEVEL		(1)

#define XBOOT_SEL_FAST_KEY		 __GPIO__(B, 4)	/* vehicle need def */

#define XBOOT_SEL_RCVR_KEY		GPIO_KEY_VOLUME_INC	/* vehicle need def */
/*======================================================================
 * Battery  QYANG
 */
#define GPIO_OTG_ID_PIN			(32 * 5 + 14)	/* PF14 for vehicle */
#define GPIO_CHARG_DETE_N		-1		/* null for vehicle */
...
#define __battery_do_charge()		do { } while (0)

#define __battery_dont_charge()	do { } while (0)
/*======================================================================
* vehicle has not pmu
*/
...
* LCD backlight
*/
#define LCD_PWM_CHN             0			/* pwm channel 0 for vehicle */
#define LCD_PWM_FULL            256
#define DEFAULT_BACKLIGHT_LEVEL 80

#define LCD_PWM_PERIOD		5000000			/* pwm period in ns vehicle need def */
#define PWM_BACKLIGHT_CHIP	1			/* 0: digital pusle; 1: PWM */
/*======================================================================
 * gpio for lcd
 */
#define LCD_PWM_PIN		(32*4 + 0)		/* PE0 for vehicle*/
...
#else
#define LCD_LR_PIN		(GPIO_NULL)		/* null for vehicle */
#define LCD_UD_PIN		(GPIO_NULL)		/* null for vehicle */
#endif
#define LCD_MODE_PIN  		GPIO_NULL

#define LCD_RESET_PIN		GPIO_NULL

#define LCD_VCC_EN		GPIO_NULL        	/* null for vehicle */
#define LCD_DE_PIN		(32 * 2 + 9)       	/* PC09 for vehicle */
#define LCD_VSYNC_PIN		(32 * 2 + 19)		/* PC19 for vehicle */
#define LCD_HSYNC_PIN		(32 * 2 + 18)		/* PC18 for vehicle */
#define GPIO_LCD_POWERON	LCD_VCC_EN
#define GPIO_LCD_PWR_N		GPIO_LCD_POWERON
#define LCD_STBY_PIN		GPIO_NULL

#define GPIO_LCD_PWM		LCD_PWM_PIN		/* PE0 for vehicle*/
#define GPIO_LCD_DISP_N		LCD_RESET_PIN		/* LCD_DISP_N use for lcd reset */
#define GPIO_LCD_POWER		LCD_VCC_EN		/* LCD_VCC_EN use for lcd reset */
#define GPIO_LCD_STANDBY 	LCD_STBY_PIN		/* LCD_STBY_PIN use for lcd reset */
#define GPIO_LCD_L_R		LCD_LR_PIN		/* null for vehicle */
#define GPIO_LCD_U_D		LCD_UD_PIN		/* null for vehicle */
