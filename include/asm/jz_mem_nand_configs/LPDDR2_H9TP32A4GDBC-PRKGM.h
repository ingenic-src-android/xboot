...
#define DDR_tRC 	({unsigned long tck = DDR_tRP;			\
			ck ? ADD(tck, DDR_tRAS) : (DDR_tRAS + tck);}) /* ACTIVE to ACTIVE command period to the same bank.*/
...
#define DDR_tRL  	6	/* LPDDR2: Read Latency  - 3 4 5 6 7 8 , tck*/
#define DDR_tWL		3	/* LPDDR2: Write Latency - 1 2 2 3 4 4 , tck*/
#define DDR_tCCD 	2	/* CAS# to CAS# command delay , tCK*/
#define DDR_tRTW 	(DDR_tRL + (((DDR_tDQSCKMAX * 1000) % ps) ?(DDR_tDQSCKMAX * 1000 / ps + 1) : (DDR_tDQSCKMAX * 1000 / ps)) + DDR_BL / 2 + 1 - DDR_tWL)	/* read to write */
