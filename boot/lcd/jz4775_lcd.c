...
#define BYTES_PER_PANEL	 (((info->modes->xres * jzfb_get_controller_bpp(info->bpp) / 8 + 3) >> 2 << 2) * info->modes->yres)
	...
	framedesc->ldcmd = LCDC_CMD_EOFINT | LCDC_CMD_FRM_EN | LCDC_CMD_SOFINT;
...
static void jzfb_config_smart_lcd_dma(struct jzfb_config_info *info)
{
	...
	//framedesc->fdadr = virt_to_phys((void *)info->dmadesc_cmd); /* DMA_CMD0 <<==>> DMA_FB, empty cmd buffer */
	framedesc->fdadr = virt_to_phys((void *)info->dmadesc_cmd_tmp); /* DMA_CMD1 <<==>> DMA_FB, , with write_gram_cmd cmd buffer */
	framedesc->fsadr = virt_to_phys((void *)info->screen);
...
#define DMA_CMD_LENGTH_IN_WORDS (1)
		...
		framedesc_cmd[1]->ldcmd = LCDC_CMD_CMD | LCDC_CMD_FRM_EN | DMA_CMD_LENGTH_IN_WORDS;
		framedesc_cmd[1]->cmd_num = cmd_num_per_word*DMA_CMD_LENGTH_IN_WORDS;
	}


	//info->fdadr0 = virt_to_phys((void *)info->dmadesc_cmd); /* DMA_CMD0 -->> DMA_FB <<==>> DMA_CMD0*/
	info->fdadr0 = virt_to_phys((void *)info->dmadesc_cmd_tmp); /* DMA_CMD1 -->> DMA_FB <<==>> DMA_CMD1*/
